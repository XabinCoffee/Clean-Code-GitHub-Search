...
import com.xabin.searchgithub.users.FetchUsersProfileUseCase
...
@HiltViewModel
class ProfileViewModel @Inject constructor(@ApplicationContext private val application: Context, val gitHubApi: GitHubApi, val fetchUsersProfileUseCase: FetchUsersProfileUseCase): ViewModel() {
    ...
    fun getUserProfile(username: String) = viewModelScope.launch(CoroutineName("getUserProfile")) {
        ...
        launch(CoroutineName("Profile Details")) {
            ...
            detailsLoading = true
            try {
                val userResp = fetchUsersProfileUseCase.fetchUserProfile(username)
                user = userResp
        ...
        launch(CoroutineName("Followers")) {
            ...
            try {
                val followersResp = fetchUsersProfileUseCase.fetchUserFollowers(username)
                followers.clear()
        ...
        launch(CoroutineName("Repositories")) {
            ...
            reposLoading = true
            try {
                val reposResp = fetchUsersProfileUseCase.fetchUserRepos(username)
                repos.clear()
