...
class UsersTestData {
    companion object {
        ...
        fun getUserSchemas(): List<UserSchema> {
            return listOf(
                UserSchema(id = 1, login = "username1",  name ="user 1", url =  "url1"),
                UserSchema(id = 2,login = "username2", name = "user 2", url = "url2")
            )
        }

        fun getUsers(): List<User> {
            return listOf(
                User(1,"username1", "user 1", "url1"),
                User(2,"username2", "user 2", "url2")
            )
        }
